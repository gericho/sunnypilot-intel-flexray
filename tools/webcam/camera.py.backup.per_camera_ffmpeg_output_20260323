...
class Camera:
  def __init__(self, cam_type_state, stream_type, camera_id):
    ...
    backend_key = "WEBCAM_BACKEND"
    if cam_type_state == "driverCameraState":
      backend_key = "WEBCAM_DRIVER_BACKEND"
    elif cam_type_state == "wideRoadCameraState":
      backend_key = "WEBCAM_WIDE_BACKEND"
  ...
  def _open_ffmpeg(self):
    ...
    split_road_cam = os.getenv("WEBCAM_SPLIT_ROAD_CAM", "11").strip()
    split_road_path = split_road_cam if split_road_cam.startswith("/dev/video") else f"/dev/video{split_road_cam}"
    is_loopback_road = self.cam_type_state == "roadCameraState" and self.device_path == split_road_path
    if not is_loopback_road:
      cmd += [
        "-fflags",
        "nobuffer",
        "-flags",
    ...
    output_pix_fmt = "bgr24" if self.ffmpeg_output == "bgr24" else "nv12"
    cmd += [
      "-pix_fmt",
      output_pix_fmt,
      "-f",
  ...
  def _read_ffmpeg_bgr(self):
    self._open_ffmpeg()
    if self.ffmpeg_proc is None or self.ffmpeg_proc.stdout is None:
      return
    frame_size = self.w_int * self.h_int * 3
    while True:
      t0 = time.perf_counter() if self.profile else 0.0
      data = self.ffmpeg_proc.stdout.read(frame_size)
      read_ms = (time.perf_counter() - t0) * 1000.0 if self.profile else 0.0
      ...
      if not self._raw_nv12_probed:
        self._raw_nv12_probed = True
        self._raw_nv12_active = False
        print(f"[webcamerad:{self.cam_type_state}] raw_nv12=inactive")
      frame = np.frombuffer(data, dtype=np.uint8).reshape((self.h_int, self.w_int, 3))
  ...
  def read_frames(self):
    if self.backend == "ffmpeg":
      if self.ffmpeg_output == "bgr24":
        yield from self._read_ffmpeg_bgr()
      else:
