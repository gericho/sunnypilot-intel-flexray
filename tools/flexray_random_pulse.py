...
def main():
  ap=argparse.ArgumentParser(description='Raw random FlexRay override sender')
  ap.add_argument('--serial', default='')
  ap.add_argument('--address', type=int, default=96)
  ...
  if not (1 <= args.length <= 32):
    raise SystemExit('length out of range')
  rng = os.urandom if args.seed is None else None
  ...
  try:
    ...
    state = args.seed if args.seed is not None else 0
    for i in range(steps):
      if rng is not None:
        payload = rng(args.length)
      else:
        data = bytearray()
      ...
      print(f'i={i:03d} addr={args.address} base=0x{args.base:02x} payload={payload.hex()}')
      if args.run:
