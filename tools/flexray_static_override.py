#!/usr/bin/env python3
import argparse
import struct
import time
import usb1

CRC8_TABLE = [
  ,
]
USB_VIDS = (0xBBAA, 0x3801)
USB_PIDS = (0xDDEE, 0xDDCC)
USB_WRITE_EP = 3

def crc8(data: bytes, init_value: int = 0xF1) -> int:
  crc = init_value & 0xFF
  for b in data:
    crc = CRC8_TABLE[crc ^ b]
  return crc

def pack_override(frame_id: int, base: int, dat: bytes) -> bytes:
  payload_len = len(dat)
  out = bytearray([0x90, frame_id & 0xFF, (frame_id >> 8) & 0x07, base & 0xFF])
  out.extend(struct.pack('<H', payload_len))
  out.append(crc8(dat[1:]))
  out.extend(dat[1:])
  return bytes(out)

def open_dev(serial=''):
  ctx = usb1.USBContext(); ctx.open()
  for d in ctx.getDeviceList(skip_on_error=True):
    if d.getVendorID() in USB_VIDS and d.getProductID() in USB_PIDS:
      s = d.getSerialNumber()
      if serial and s != serial:
        continue
      h = d.open(); h.claimInterface(0)
      return ctx, h, s
  ctx.close()
  raise RuntimeError('no picoflex device')

def main():
  ap = argparse.ArgumentParser(description='Static FlexRay override sender using validated pack format')
  ap.add_argument('--serial', default='')
  ap.add_argument('--address', type=int, required=True)
  ap.add_argument('--base', type=lambda x: int(x, 0), default=0x01)
  ap.add_argument('--payload-hex', required=True)
  ap.add_argument('--hz', type=float, default=10.0)
  ap.add_argument('--seconds', type=float, default=10.0)
  ap.add_argument('--run', action='store_true')
  args = ap.parse_args()
  payload = bytes.fromhex(args.payload_hex)
  ctx, handle, serial = open_dev(args.serial)
  try:
    print(f'connected picoflex: {serial}')
    print(f'addr={args.address} base=0x{args.base:02x} payload={payload.hex()}')
    pkt = pack_override(args.address, args.base, bytes([args.base]) + payload)
    if not args.run:
      print('dry-run only; add --run to transmit')
      return 0
    period = 1.0 / max(args.hz, 1.0)
    steps = max(1, int(round(args.seconds / period)))
    start = time.monotonic()
    for i in range(steps):
      handle.bulkWrite(USB_WRITE_EP, pkt, timeout=10)
      next_t = start + (i + 1) * period
      sleep_s = next_t - time.monotonic()
      if sleep_s > 0:
        time.sleep(sleep_s)
    return 0
  finally:
    try:
      handle.close()
    finally:
      ctx.close()

if __name__ == '__main__':
  raise SystemExit(main())
