...
from openpilot.sunnypilot.pc_runtime.helpers import is_pc_webcam_runtime, get_selfdrived_init_timeout, get_selfdrived_packets
...
class SelfdriveD(CruiseHelper):
  def __init__(self, CP=None, CP_SP=None):
    ...
    if is_pc_webcam_runtime():
      # PC webcam rigs don't provide clean stock-quality validity for these
      # learned/location/helper topics in this custom setup. Exclude them from
      # generic selfdrived health gating so they don't block engagement.
      ignore += ['liveCalibration', 'livePose', 'liveDelay', 'liveParameters', 'liveTorqueParameters',
                 'driverAssistance', 'longitudinalPlanSP', 'modelDataV2SP', 'alertDebug']
  ...
  def update_events(self, CS):
    ...
    if not REPLAY and self.rk.lagging and not is_pc_webcam_runtime():
    ...
    if not self.CP.notCar:
      ...
      if not is_pc_webcam_runtime() and not self.sm['livePose'].inputsOK:
        self.events.add(EventName.locationdTemporaryError)
      if not is_pc_webcam_runtime() and not self.sm['liveParameters'].valid and cal_status == log.LiveCalibrationData.Status.calibrated and not TESTING_CLOSET and (not SIMULATION or REPLAY):
