...
class ONNXModelRunner(ModelRunner, SupercomboTinygrad, PolicyTinygrad, VisionTinygrad, OffPolicyTinygrad):
  ...
  def prepare_inputs(self, imgs_cl: CLMemDict | NumpyDict, numpy_inputs: NumpyDict | None = None, frames: FrameDict | None = None) -> dict:
    if numpy_inputs is None and frames is None:
      numpy_inputs = imgs_cl  # backward-compatible tinygrad-style call site
      imgs_cl = {k: v for k, v in self.inputs.items() if k in self.vision_input_names}
...
class ONNXSplitRunner(ModelRunner):
  ...
  def prepare_inputs(self, imgs_cl: CLMemDict | NumpyDict, numpy_inputs: NumpyDict | None = None, frames: FrameDict | None = None) -> dict:
    if numpy_inputs is None and frames is None:
      numpy_inputs = imgs_cl  # backward-compatible tinygrad-style call site
      imgs_cl = {k: v for k, v in self.inputs.items() if k in self.vision_input_names}
      frames = getattr(self.vision_runner, "frames", getattr(self, "frames", {}))
    t0 = time.perf_counter()
